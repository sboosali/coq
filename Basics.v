...
Fixpoint plus (n : nat) (m : nat) : nat :=
  match n with
    | O => m
    | S n' => S (plus n' m) (* i like this better,  plus n' (S m) *)
  end.
...
Theorem plus_1_neq_0_firsttry : forall n : nat,
  beq_nat (n + 1) 0 = false.
Proof.
  intros n. destruct n. simpl.  (* does nothing! *)
Admitted.
...
(* Notation "x & y" := (andb x y) (at level 65, left associativity) : bool_scope. *)
(* Notation "x  y" := (orb x y) (at level 70, right associativity) : bool_scope. *)
...
(* destruct adds "var = some case" to context, while induction adds either "var = basecase" or "var = recur AND curr given prev" *)
...
induction m.
 ...
 induction n.
  simpl. apply IHm.
  simpl. rewrite-> IHm. simpl. rewrite-> plus_swap. rewrite<- IHn. simpl. rewrite-> IHm. reflexivity.
Qed.
(** [] *)
...
Notation "x == y" := (beq_nat x y)        (at level 60, right associativity) : nat_scope.
Notation "x \= y" := (negb (beq_nat x y)) (at level 60, right associativity) : nat_scope.
Notation "x ≤ y"  := (ble_nat x y)        (at level 60, right associativity) : nat_scope.
...
intros n m.
induction p.
 intro A. simpl. assumption.
 (* modus ponens *) simpl. assumption.
Qed.
...
intros n m.
induction p.
 rewrite mult_0_r; rewrite mult_comm. rewrite mult_0_r.
 reflexivity.
 (**)
 apply (plus_comm (n * S p) (m * S p)).
 replace (n * S p) with (S p * n).
 replace (m * S p) with (S p * m).
 replace ((n + m) * S p) with (S p * (n + m)).
 simpl. rewrite mult_comm. rewrite IHp. rewrite plus_assoc.
 replace (n*p) with (p*n).
 replace (m*p) with (p*m).
 (* n + m + p * n + p * m  ==  ((n + m) + p * n) + p * m
    <--  "+" : left-associative
 *)
 replace ((n + m) + p * n) with (n + (m + p * n)).
 replace (m + p*n) with (p*n + m).
 replace ((p*n + m) + p*m) with (p*n + (m + p*m)).
 (* set x = p*n
    for readability
 *)
 replace (n + (p*n + m)) with ((n + p*n) + m).
...
Inductive bin
inc
inc  commutes with  binary=>unary
*)
...
outer constructor  =  least significant bit
*)
...
Example t_power_of_two: 2^4 = 16.
Proof. reflexivity. Qed.

Fixpoint _b2u (i:nat) (n:bin) : nat :=
  match n with
    | o => O
    | _1 n' => 2^i + _b2u (i+1) n'
    | _0 n' =>       _b2u (i+1) n'
...
Fixpoint b2u (n:bin) : nat := _b2u 0 n.

Example binary_to_unary: 
 andb (b2u (_1 (_1 (_1 o))) == 7)
      (b2u (_0 (_0 (_0 (_1 o)))) == 8)
= true.
Proof. reflexivity. Qed.
...
| S m' => match n with
            | O => Ackermann m' 1
            | S n' => O (* Ackermann m' (Ackermann m n') *)
          end
end.

(*
Theorem play : theorem
Proof.
proof
Qed.
*)
...
(* Locate "_ & _".
if theres only 1 "&", why cant i use it unambiguously?
*)
