...
Proof.
(* set x2 to n in the env, then get x3 which isnt in it *)
intros. unfold update. auto. 
Qed.
...
Proof. intros. unfold update.
remember (beq_id k1 k2). destruct b.
...
remember (beq_id k1 k3) as A. remember (beq_id k2 k3) as B.

destruct A.

 apply beq_id_eq in HeqA;
 destruct B; subst.
  apply beq_id_eq in HeqB. contradiction.
  reflexivity.

 symmetry in HeqA. apply beq_id_false_not_eq in HeqA.
  destruct B; subst; reflexivity.
...
(* induction principle i want *)
Inductive no_whilesR: com -> Prop := 
| nwSkip : no_whilesR CSkip

| nwAss  : forall (i:id) (a:aexp),
 no_whilesR (CAss i a)

| nwSeq  : forall (x: com) (y:com),
 no_whilesR x ->
 no_whilesR y ->
 no_whilesR (CSeq x y)
 
| nwIf   : forall (b:bexp) (t: com) (f: com),
 no_whilesR t ->
 no_whilesR f ->             
 no_whilesR (CIf b t f)
.
...
match prog with
  ...
  | i::is =>
    ...
    match i with
      | SPush n => s_execute st (n::stack) is
      | SLoad x => s_execute st (st x :: stack) is

      | SPlus   =>
        (* (let (x,y,stack) := popop stack in *)
        (*  s_execute st (y+x :: stack) is) *)
        match popop stack with
          | (x,y,stack) => s_execute st (y+x :: stack) is
        end
...
(** The task of this exercise is to prove the correctness of the
    ...
    Prove the following theorem, stating that the [compile]
...
(** Prove the following lemmas about your definition of [ceval]: *)
...
(* <$Date: 2013-02-20 09:06:42 -0500 (Wed, 20 Feb 2013) $ *)
